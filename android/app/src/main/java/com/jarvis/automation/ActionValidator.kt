...
object ActionValidator {
    ...
    private val policies = mapOf(
        "open_app" to ActionPolicy("open_app", RiskLevel.AUTOMATIC),
        "read_screen" to ActionPolicy("read_screen", RiskLevel.AUTOMATIC),
        "swipe" to ActionPolicy("swipe", RiskLevel.AUTOMATIC),
        "wait" to ActionPolicy("wait", RiskLevel.AUTOMATIC),
        "go_back" to ActionPolicy("go_back", RiskLevel.AUTOMATIC),
        "go_home" to ActionPolicy("go_home", RiskLevel.AUTOMATIC),
        "tap" to ActionPolicy("tap", RiskLevel.LOW),
        "type" to ActionPolicy("type", RiskLevel.LOW),
        ...
        "calendar_search" to ActionPolicy("calendar_search", RiskLevel.LOW),
        ...
        "send_sms" to ActionPolicy("send_sms", RiskLevel.HIGH, requiresConfirmation = true),
        "share_text" to ActionPolicy("share_text", RiskLevel.MEDIUM),
    )
    ...
    fun validate(action: JSONObject): ValidationResult {
        ...
        if (policy == null) {
            Log.w(TAG, "Unknown action type: $type — allowing by default")
